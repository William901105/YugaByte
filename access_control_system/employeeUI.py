...
SESSION_FILE = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), "employee_session.json")
...
def query_records():
    ...
    # 計算今天的開始時間 (00:00:00)
    current_time = time.time()
    today_start = time.strftime(
        "%Y-%m-%d 00:00:00", time.localtime(current_time))
    today_start_timestamp = time.mktime(
        time.strptime(today_start, "%Y-%m-%d %H:%M:%S"))

    # 處理預設值和用戶輸入
    start_time_input = input(f"開始時間 (預設: {today_start}): ")
    start_time = parse_datetime(
        start_time_input) if start_time_input else today_start_timestamp

    end_time_input = input(f"結束時間 (預設: {format_timestamp(current_time)}): ")
    end_time = parse_datetime(
        end_time_input) if end_time_input else current_time
...
def view_notifications():
    ...
    if not warning_messages:
        ...
        sorted_notifications = sorted(
            warning_messages, key=lambda x: x["time"], reverse=True)
